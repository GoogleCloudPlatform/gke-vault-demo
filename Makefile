...
# 1. Creates the vault and application gke clusters in separate projects
.PHONY: create
create:
	@source scripts/create.sh

# 2. Ensures that the Vault HTTPS endpoint is healthy and that a workload
#    in the application cluster can fetch secrets from Vault.
.PHONY: validate
validate:
